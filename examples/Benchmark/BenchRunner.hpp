...
public:
  explicit BenchRunnerBase(const BenchConfig &benchConf)
      : m_testMatrix{generateTestMatrix<MatrixType>(benchConf)},
        m_benchConfig{benchConf}, m_stats{} {}
...
public:
  ...
  void run() final {
    ...
    const MatrixType reconstructed{svd.matrixU() * svd.singularValues().asDiagonal() *
                                   svd.matrixV().adjoint()};
...
public:
  ...
  void run() final {
    ...
    const MatrixType reconstructed{rsvd.matrixU() * rsvd.singularValues().asDiagonal() *
